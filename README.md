...
- 第三小问 LC337： 树形dp
  ...
  - 员工的快乐值类似题
  - 【错题】
  - 对于当前节点，有两种选择（以左孩子的info为例）：
    - (1) yes -> 只能选择 左孩子.no    （因为不能相邻选）
    - (2) no 【错误点】 -> 两个选择都可以，既可以选择 左孩子.yes，也可以选择左孩子.no，但是只能选一个，所以 =》 选择 二者之中的最大值
    - 最后返回时，比较头结点返回的info中，yes,no 中较大值即可
...
- 【错误点】注意 一定要先算 前面有没有 preSum - targetSum满足的前缀和，然后再把自己当前的preSum加到map中（防止出现target=0要求，会错），后续继续dfs。
```
...
- 欠账本的算法？
  ...
  - 欠账时，
    - 一个很简单的分辨，如果 操作完之后， map.get(欠账符号) > 0， 那么 all++，否则不变
...
- 【错误点】一定注意，比较的是 mostRight.right!!!! 不是mostRight, 永远比较的是 左树最右节点的 右指针 （来判断 拥有左树的节点是第一次到达还是第二次到达）
...
- LC114. 二叉树展开为链表
  ...
  - 综合上述两点，我们说 在先序遍历时，通过左指针 串起 先序遍历，（注意 右指针不能变动）。 然后（按照题目的要求，左指针指向null，右指针指向先序遍历下一个节点，）所以 我们最后 再单独 指针逆转就可以了。
...
- https://leetcode-cn.com/problems/er-cha-sou-suo-shu-yu-shuang-xiang-lian-biao-lcof/
```
核心就是 Morris中序遍历 改链表时， 当前结点的左指针随便改， 
而当 节点跳转到下一个节点时（pre = cur, cur = cur.right），
这个 pre节点的右指针 是可以进行更改的，不会造成错误。 因为 修改左指针的时候，
右指针不变，然后跳向正确的右指针。 当跳到下一个节点时，
上一个节点的右指针已经无用了，所以可以修改 Pre 的右指针。
也可以举例来画。
```
- 综述就是，可以正常的在每个节点进行 pre.right=cur, cur.left = pre, pre = cur 的操作。 不需要特别处理。
...
- 递归 + 非递归（class Op记录左右边界，用户栈模拟系统栈存储）

[2 归并排序](sort/MergeSort.java)
- 递归 + 非递归（while + 步长）

[3 堆排序](sort/HeapSort.java)
- 只需要使用 【堆的向下调整】就可以



### 有序表
- 关于有序表改写，SB树该有序表非常的方便
- 改写的原因？ 系统提供的有序表中不包含一个功能 <=k 的 key 有几个？
  - 所以我们要自主实现 有序表 来实现这个功能
- 平衡搜索二叉树也好，其他有序表也好，都是 为了让 整个数据结构的 数据分布规律 不受 用户的输入规律影响，从而优化性能。
- 很有意思的一点就是，单纯的 LR型，第一步之后是为了让 子树变成LL型，
  - 同理 RL型，执行完第一部 右旋之后，就变成了RR型，从而就可以 左旋



[1. AVL树]
  - 控制左右子树的规模，使左右子树的规模相似。
  - 左右子树高度差 <= 1
  - 平衡因子为 【高度】
...
[2. SB树]
  ...
  - 平衡因子为 【size】
  - 原则： 【注意点】不同于AVL树调平一次后就可以返回（调父节点），调平操作后，谁的孩子发生变化了，递归调用m(a)去调平孩子改变的节点。
    - AVL树平衡性非常敏感，每次加减节点都会影响节点往上一流的节点。 所以每次调一次就可以调。 不会出现集体大雪崩的时刻，因为 每次AVL一有不平衡就调了。
    - 而SB树，只关心 节点个数之间的关系，所以就有可能很久不需要调一次，所以当某个突然出现雪崩的时刻，就需要这样的递归往下的动作来平衡。 严格数学证明后，时间复杂度依然是 O（logN）
      - 而当我在该节点调平之后，我并不能确定 我底层节点是否还是有效的。为什么时间复杂度还是有效的呢？ 均摊这件事情。 之前剩下来的动作，即便这一次学崩了，均摊下来 时间复杂度 还是 O（logN）
  - 【核心点在于与AVL树的不同调平操作】在SB树的调整中，只需要记住一点，LL LR RL RR型违规，它的调整动作与AVL树一样，不同点在于 调整完之后要审视一下，谁的孩子变了，要去做后续的递归调平操作。
  - 剩下的跟AVL一样，加节点的时候，从受影响节点开始，沿途所有节点都要 检查（m）一遍。
    - 删除节点一样，从受影响节点往上，沿途节点全部 m 一遍
  - 【优势】SB树 可以 不在删除的时候 加 平衡性的调整。  只在加入节点的时候，做平衡性的调整。   因此sb树可以
    - 即便删除了 棒状结构，所以 时间复杂度就是  插入最多数据量情况下的 N -> O(logN)
      - 而且一旦在 加入节点后，就又会调平了。
    - 充分考虑到 能少调就少调。
- SBT多一个小功能 -》 可以 返回 排序后 对应下标位置的 Key(因为通过KEY就能够找到value)
  - 左子树的大小就是 当前节点的 相对下标
  
- 积压结构
  - 我在某一个时刻会 迎来一个性能瓶颈， 比如ArrayList扩容时
  - 但是 平摊下来 就是 O（1）
  - 同样的道理， HashMap 也是这个样子
  - AVL就不是积压结构，  SB树 红黑树都是积压结构，就都是 单点都会迎来一个瓶颈时刻，但是 整体能够 收敛到 O（logN） 的水平
  - 积压结构，模糊平衡性 -> 很久 处理一次
  
[2.1 SBT 实现数组](orderedlist/ReconstructQueue.java)
- 实现数组功能， 插入 查询 删除 都是 O（logN）
- LC406. 根据身高重建队列
```
 使用 SBT
 ...
 （2）身高 （由大到小排）   身高相同时，ki由小到大
 组成序列后，
 [hi,ki] 其中 ki就是要插入的位置，依次插入即可
 （3）【核心优化】我们优化 插入操作， SBT改  -> O(logN)， 最终时间复杂度就变成了 O（N * log N）
```
  
[3. 跳表]
- 多层链表
- 【核心点】就是 从最高层开始，往右跳到不能再跳，然后降一层，再往右到不能再往右
- 第一层： N  -> N / 2 -> N / 4
- 每一个新的数字， 扔硬币， 百分之50概率往上走， 百分之50概率停在本层
- 每一个数字随运气决定高度
- 这使得最终形成跳表的样子 与 输入规律无关
- 用概率 把 输入规律这件事情 解耦掉
  - 跳表里面的分布规律全部由概率决定， 与输入规律无关
- 从高层往右找到 < k 的最右，然后降一层往右到 < k 的最右，直到来到第0层。得到 < k的最右的点
  - 如果 mostRight.next == 目标，只更新
  - 否则，插入
    - 首先 roll 硬币，决定 新节点 的 层数
- 【删除操作】 删除之后，如果高层是 删除节点独享的，那么 最左侧的链表中要将最高层的链表也删掉
- 有序表，当要做出Key的修改的时候，就把原来的key删除，然后重新插入新的。
- 当我数据量很多的时候，上层跨过一个，下面就会跨过一批
- 最终计算的概率长期期望是 O（logN）
- 实现
  - 最左侧使用null来做，初始化第0层
- 【先进行在哪】 随机行为 避免 用户输入规律
...
[约瑟夫环问题]
- 题目描述： 给定一个链表头节点head，和一个正数m，从头开始，每次数到m就杀死当前节点，然后被杀节点的下一个节点从1开始重新数，周而复始直到只剩一个节点，返回最后的节点
- Leetcode题目（稍微改了下描述）：https://leetcode-cn.com/problems/yuan-quan-zhong-zui-hou-sheng-xia-de-shu-zi-lcof/
- 最差解法： O（M * N） -> 总共要杀n-1个节点，如果每个节点都要数数的话，那就是 n * m 的时间复杂度。O（N * M）
- 最优解 -> 约瑟夫环问题
  - 【核心】创建一个 f（） 函数， 参数是 在执行杀节点操作后，长度为i - 1的情况下，某个存活节点的编号， 返回的是 杀节点之前，长度为 i时， 该存活节点的编号
    - 先看下面，再看这个 -> 抽象化出函数图像，依然是一个 【剃刀感觉】的 图像
    - 但是 需要 左部分延长到1，右部分延长的i，才能形成剃刀， 那么怎么实现这个 延长之后才像剃刀的函数呢？  把定义域规定好就可以了！ 该怎么延长怎么延长，定义域给你限制住就可以了。
    - 公式， 对比下面的函数， 观察可知，就是 下面的函数 向左移动 s 位得到的！ （s表示 当前删除节点的编号）
    - 所以结果就是 (杀之后的编号 - 1 + s) % i + 1;    (i 为 当前总长度)
     - s 是什么呀？ s 是 长度为 i 的链表中，数到m就杀掉的节点编号。
  - 【先看g() 再看 f()】再创建一个 g() 函数， 参数是 传进来的一个数字m（轮到m次就杀掉节点， m可以>i）， 返回的是 这个该删除的节点在哪个 编号（的节点）。 前提，在长度为i时，要轮到m次删除节点
    - 基于基础变换： x % i  ==》 左加右减（x的+-），上加下减（外部系数的+-）
      - 于是乎，就求出来 y = (x - 1)  % i + 1;  (因为 要求图像是 向右移动一位，向上移动一位)
      - 剃刀感觉的函数， 都从 x % i 出发
  - 综上所述
    - 针对 f() 公式， 我们要求出s， 那么因为 s就是长度为i 的链表中的 每m个就杀掉的编号，所以将m带入公式g()
      - 所以 s = (m - 1) % i + 1, 带入 f() 公式
      - 此时，整个公式 只有 m 和 i
      - 最后可以化简成 前 = （后 + m - 1） % i + 1 
        - 原公式是 前 = （后 + (m - 1) % i） % i + 1
      - 【含义】这个公式就是，“后” 表示 在长度为i-1时，存活节点的编号，可以求出 在长度为i时，该存活节点的编号
      - 递归/迭代 （从1往上推，推到 长度为i）
  - 这类变形题： 都从 x % i 这样的 剃刀感觉的函数 去推
    - 变形题题目： 有一些小伙伴，比如五个，围成一圈，轮流抽数字，第一轮抽到数字7，抽到数字7淘汰，第二轮抽到数字3，抽到数字3的淘汰，第三轮抽到数字100,抽到100淘汰，问： 那个小伙伴会活？
    - 还是那个公式， 无非就是 约瑟夫环的m是固定的，这道题的m是不固定的，只需要 写代码时把 m依次读出来就可以了，依然是那个公式！
  -【注意】我们是从1下标开始的，对于leetcode那道题，最终得到答案 -1，就可以了（因为lc 是从0开始）
### 字符串相关问题
...
### 动态规划 DP经典题
关于DP：
1. dp 对应 题目求得答案解的映射关系（基本上）
2. dp[i][j] 对应 A对象的i映射的某个含义 与 B对象的j映射的某个含义， 基于 题目解的映射关系 的答案值
3. 【重要】本质上是 递归思想， 只是 通过 状态转移方程来 推
4. 【重要续】 状态转移方程 本质上 就是 递归的 **【分解子问题】**， 所以 答题的关键 就是 【分解子问题！！！】， 将 dp[i][j]分解成子问题就可以了

（1）样本对应模型 （编辑距离，最长公共子串）
- 1. dp[i][j] 表示 str1 长度为i的前缀 与 str2 长度为j的前缀的 某某关系
- 2. 针对 str1[0 - i-1] str2[0 - j-1] 的 结尾字符 做文章

（2）从左往右的尝试模型的 直接dp用法，想想背包问题。 dp[i][j] 表示 i及其往后的范围，搞定j这么多的rest剩余数量
    - 那么 i 的依赖一定就是 i + 1，然后就是 对应 j这个 rest的 状态去憋就行了（比如说j就是剩余量，那么剩余量会减小）
    - 拿零钱兑换举例，
      - 再次重申，dp[i][j] 对应 dp[index][rest] 对应 Process(index, rest), 表示 index及其往后，处理rest这么些数量。 所以 index 必 依赖于index+1 ，而状态转移，基于当前的index，对 rest进行 +-， 结果传到 process(index + 1, rest - arr[index]*m)  ==> dp[i][j - arr[i] * m]

（3）对于很多dp题而言，分析关键就是 对于 dp[i]而言， i位置 【参与】 还是 【不参与】，这样的思路来指示如何分析状态转移
...
[32. 最长有效括号](DPClassicProblems/LongestValidParentheses.java):
- 看到子串问题 -> 以i位置结尾 来求局部最优解， 最终合并 所有的局部解 得到 全局解

- 错误点：
- 少考虑了 ()()(()()) 遇到最后一个字符 dp[i]时
  - 遇到')'先从 dp[i - 1]找到中间合法大小，然后找到L位置，看L是否为‘（’,是的话，则 当前 (...) 长度可得
  - 【接下来就是 错误点】还没结束！！！ 注意，dp[i]表示当前i位置的最长有效括号长度，所以 【还要看】 L - 1 >= 0时， dp[L - 1]的值，则为当前连续的有效括号的长度。
- 就是说 当 一个 ) 与前面的( 结合后， 先算出 (.....) 的距离，但是此时，还要加上 L - 1位置所算出来 的 dp
```java
int L = i - dp[i - 1] - 1;
if (L >= 0 && chs[L] == '(') {
    dp[i] = 2 + dp[i - 1];   // 当前括号长度
    if (L - 1 >= 0) {
        dp[i] += dp[L - 1];  // 加上 之前的连续的 最长有效 -> 和即为当前的 最长有效
    }
...
[LC72. 编辑距离](DPClassicProblems/EditDistance.java) [牛客 nc 35] （lc 那道题就变成 代价全为1就行了）
- 样本对应模型
...
[322. 零钱兑换](coinchange/CoinChange1.java)
- 二维DP + 无效值设置 + 无效值的特殊判断处理 + 斜率优化 + 简单空间压缩
- 本题依旧是一个 二维DP问题。
...
- 设置成最大值的话，后面计算min时候就会消去影响。 【不对】 因为 有 + 1 操作，所以会导致结果变成最小值，所以完全不对！ 必须单独讨论。 【记住】任何题中，一旦设置了 无效值，一定要 每个答案 都单独 检验
...
- 于是， 状态转移方程 -> dp[i][j] = dp[j + 1][j - coins[i] * m]   (m >= 0 && j - coins[i] * m >= 0)
- 经过观察后发现 斜率优化的方向：
...
[394. 字符串解码]
- 嵌套模型 -> 递归函数的返回值设计 【1】结果 【2】结束位置
- 因为保证一定合法： （1）数字后面一定跟括号 -> 递归函数返回字符串
- 设计一个函数： 传入 num, String，返回 解码后的字符串
- 【错题】错误点1（存在数字>10，要累加计算） + 错误点2【重点！】 （遇到[]返回时就计算 decode的情况,并且就在此时 就要计算 curNum 与 Info.ans 的结合情况，然后 curNum 清0）
...
1. 【重点】abcabck 对于这个k位置，它只关心 他前面字符串的 前缀串与后缀串的最长匹配长度
  - 而且限定 前缀串不能取到整体，后缀串也不能取到整体的情况下，这个长度是多长
  - 【注意】只看之前的字符串信息
  - k位置 存储的 next[i] 的值 与 k位置是什么没有任何关系，只关心前面的
- 模式串与主串
2. next[i] 的直观作用就是 对于模式串，当前i位置匹配失败的话，去next[i]位置继续匹配
...
[1438. 绝对差不超过限制的最长连续子数组](slidingWindow/LongestSubarray.java)
- 时间复杂度 O（N）
- 滑动窗口的最大最小值更新结构 + 舍弃可能性！ -> 以每个i位置为开头获得 局部最优， 合并起来得到全局最优，然而 舍弃可能性思想要用到！
- 只需要返回最长，所以可以舍弃可能性！
- 【错误点】注意R右移循环时的条件设置，对于窗口内没有数字时的特殊讨论，使用|| 条件， L==R || 最大值-最小值<=limit （不然会导致 qmax.peekFirst() 返回Null,造成 nums[null]的问题）
...
### indexTree
- 也是想要 解决 【频繁调用累加和】 的问题
  - 当数组永远不动的 情况下，使用 【前缀和数组】 就可以实现目的
  - 但是，如果 arr 会存在 单点的改变,前缀和数组的改动会造成非常大的性能损耗，所以就需要一个新的结构 -> index Tree
- 不同于线段树 -> **树状数组实现 单点更新 和 区间查询（实质上是返回 -> 1-index的前缀和，但是可以通过【前缀和】 加工出来 【区间和】） **
    - 时间复杂度是 O（logN）
- 【尤其要注意】下标也是从 1 开始 （必须从1开始，才会有下面的 神奇的 【实现规律】） （0位置 弃而不用）
- 神奇的实现：
  - 【核心 - index含义】（1）随意给定一个 二进制形式， 比如是 index = 010111000，那么问 这个index管了哪些值？
    - 答： 把最后一个1拆开之后的第一个数(抹掉最后一个1 然后再加1)， 一直管到它自己
      - e.g. 010110001 ~ 010111000
      - 另一个例子， 12345678， 8 -> 01000， 管的范围就是 0001 ~ 1000 （1~8）
      - 另一个例子， 123456789 10 11 12， 12管的谁， 9~12， 验证 12-> 1100    ==》 1001 ~ 1100 （9 ~ 12）
    - 【小发现】所以所有的 奇数 位置，都是只管自己的。 为什么呢？ 因为二进制形势下 第0位都是1，那么拆开之后加1，还是自己。 所以只管自己
  - 至此，随意给一个index，就知道 它负责的 【范围累加和】 了
- 【核心 - 查询】那么我想求 1-33 的累加和，怎么求呢？ ==》 需要将不同index位置的数字进行累加，因为 不同index 对应不同的 区间范围。 累加起来就可以
  - 33 的二进制形式 -> 0100001
  - 那么我只要求  0100001 + 0100000 这两个下标对应的值就可以了 => 分别对应这两个范围  [0100001 - 0100001] + [0000001, 0100000]
  - 【发现】所以，当我相求 1~index 的前缀和， 先取 原数字 + 抹掉最后一个1位置的东西 + 抹掉最后一个1位置的东西 + ... + 直到0停
    - 最右侧的1怎么取呀？
      - a & (-a) !
- 【核心 - 单点增加（更新）】
  - 假设 我要更新 3位置 （从1开头）， 那么哪些位置受牵连呢？ 
  - 123456789， 3(3-3) -> 4(1-4) -> 8(1-8), 这三个位置受牵连
  - 寻找关系  ->  0011 -> 0100 -> 1000
  - 【发现】 每次， 加上 最右侧的1， 直到 越界
  - Tip: 其实本质只能做单点增加，那么如何实现单点更新呢？ 那就需要 自己内部也建立一个数组，每次记录 上一次该位置的值，从而 计算每次 新值的增量值，从而就可以转换成 在该点 单点增加了
- 【总结】
  - 给你一个二进制的数， 01011000， 如果说这个数字上 加了一个d，那么还有哪些位置要更改呢？
    - 【从index不断的加最右侧的1】 当前位置， 当前位置 + 最右侧的1，上一个位置 + 最右侧的1 ... （越界）
  - 如果给你一个位置，我想知道 1~index的累加和是多少？
    - 【从index依次减掉最右侧的1】， 累加完就可以了
- 复杂度呢？
  - 给我一个位置，我做几次操作？
    - 有几个位信息就做几次操作。  ==》 log(N)  单点更新和求前缀和都是 O（logN）
  
- 那么 indexTree 存在的意义是什么呢？ （命名线段树都可以做）
  - indexTree 很容易可以 推到 二维,甚至三维！【几乎是唯一的优势】
  - 我们说 一维你就是在 原数组上修改一个值，那么二维呢就要求你在一个 二维数组里面把某一个值修改了。
  - indexTree解决的就是 从左上角【（1,1）位置， 0位置弃而不用】 到 一个位置作为右下角的【（i,j）】，这样一个大框
    - 具体而言，就是 创建一个 int[][] help数组 等于什么？
      - 就是 原数组中 从（1,1）到（i，j）的矩形内元素累加和的值，怎么求？
        - 与下方 更新的方法相同， 注意 update可以转化成add，因为 记录增量就可以
        - 结合下方更新的方法，我们说 只要 将 每个i去掉最后一个1的集合 * 每个y去掉最后一个1的集合， 所有的help中的值累加起来，就是 （1,1）到（i，j）的矩形内元素累加和的值
    - 那么当原位置一个位置 (i,j)的值改变了，哪些位置 受影响？
      - 假设 原数组中 行的二进制 0110100， 列的二进制 0111000， 这个位置的值被改变了
      - 那么在 help数组中，哪些相应的位置会被改变？
        - 行号管的是哪行到哪行？ 他自己 -> 不断加最右侧的1  ==》 0110100 ， 0111000， 1000000
        - 列号管的是哪些列？ 他自己 -> 不断加最右侧的1    =》 0111000， 1000000
        - 【关键】我们说， 行受影响的范围 * 列受影响的范围 （全部组合）都受影响
          - 也就是 行：【0110100 ， 0111000， 1000000】  与  列：【0111000， 1000000】 的全部组合都受到影响
    - 从而，任何一块的累加和都可以通过 相关的四块 （左上角与右下角）拼出来答案
    - 所以对于二维树状数组的 单点更新和区间查询， 时间复杂度就是 O（logN * logM）


- 三维相同道理， i,j,k 也是 他自己 -> 最右侧的1不断累加，  三者的所有组合 都受影响
...
- 是什么？
  ...
  ![img.png](img/ACAutomation.png)
- build() 使用 BFS建立 AC自动机
  - 父节点 遍历的时候 更新孩子节点的 fail指针  
- 具体细节都写在代码中
...
[15 三数之和](TopInterviewClassicQuestions/ThreeSum.java)
- 基于 有序数组的二和问题  -> 双指针
  - 去重问题
    - （1）过滤去重
    - （2）流程上 通过设计 让它就没有重复值 ！
...
[31. 下一个排列](TopInterviewClassicQuestions/NextPermutation.java)
- 【核心】就是 寻找最右的较小值，以及 较小值右侧最小的较大值。 目的都是是让 增大的范围尽可能小 （所以从右找第一个较小值）
- Step1: (从右往左找第一个下坡)从右往左寻找第一个 【较小值】 (arr[i] < arr[i + 1])  （可以发现 【较小值】 右侧 全部是 降序排列）
  - S1: 为使变化最小，所以从右往左找 第一个 较小值 （这种情况下，右侧全降序说明是最大的，下一个排列 就要 进位然后变成最小了）
  - 这个 【较小值】 下坡 的位置 就是要 进位的位置 （因为右边是 对应区间的最大）
- Step2: 从右往左寻找第一个 >【较小值】的 【较大值】  （这样的话，就是寻找到了 【较小值】 右侧 >较小值 的最小的较大值）
- Step3: 将 【较小值】 与 【较大值】 下标数值交换
- Step4: 交换完成之后，当前总值一定变大，为了让 增大范围最小，所以 要将 交换后的【较大值】 右侧区间 升序排列（这样就是最小）
  - 但是这里只需要双指针逆序即可，因为 交换之后，【较大值】右侧 依然是 降序排列数组， 逆序即可 变成升序数组
- 【特殊情况下】如果 Step1中 没有找到 【较小值】（即每一个左边的数字都比右边大，从右往左全升序），那么说明此时 arr 就是 最大字典序排列
  - 那么 nextPermutation 就是 最小的排列喽， 直接全逆序就可以
- 【错误点】
```
...
[763. 划分字母区间]
- 1. 先搞一张map，记录 每一个字符在字符串中 最靠右的位置 (【注意】因为全是小写字母，代码实现中 使用 int[26] 实现 映射map 提升 60% 速度)
  - 指示当前块的右边界 必须在哪
...
```
- 假设 大 - 小 == s， 到达第i轮时， 给 小的a 这么多，给 大的b 这么多， 假设到达第i轮的时候 a - b == s
  - 其中 a > 0, b >= 0 (因为b是给小的 的部分，所以有可能为0)
- 接着两个推理
  - a - b == s
  - a + b = 1->i的累加和， 也就是 (i * (i + 1)) / 2, 我们称之为sum;
- 从而就得到了 
  - a = (s + sum) / 2  =》 > 0 的整数
  - b = (sum - s) / 2 =》 >= 0 的整数
    - 从而我们基于b就推出了 sum - s  是一个 >= 0 的偶数
- 于是乎，问题就转化成了 如何使得 sum - s 是一个 >= 0 的偶数
  - 因为要求最少轮i，我们分为两步求
    - （1）先让 sum >= s, 二分法（1,2,4,8,16），先找到 最小的 >= s 的sum
      - 假设，在 16 没有追上，在32追上了，那么我们可以知道 最小的答案一定在 16 - 32 之间
    - （2）然后再在这个 粗粒度范围内 寻找精细范围 =》 依然是使用二分法，找到 >= s 的 sum(i)
    - (3) 现在已经找到一个合适的i了，假设i=47,那么我们现在已经sum追上s了，再去解决 sum - s 是一个 >= 0 的偶数的问题
      - 现在就要 47 48 49 50 依次试 
      - 【关键】我们发现，最多只需要尝试三次就可以试出来了。
        - 因为 sum的奇偶性是 奇奇偶偶奇奇，所以只要找到 sum >= s 且 sum 与 s奇偶性一致的时刻
        - 就能够实现 sum - s >= 0的偶数了。 所以 最多试三次。
[2]长城守卫军
- 技巧点
  - 二分答案  ->   找出 最大值（最大值 + 将军覆盖的值）和最小值（arr中最小值以及将军不覆盖的情况下）的范围区间 -> 这是答案存在的 所有可能性区间
    - 然后 对这个区间进行二分，创建一个函数 boolean f(..., limit) 表示 能够达到limit，可以true,不行false， 最后寻找最大的true就可以了（true就记录往右找，false就往左找）
      - 遍历每个位置，如果arr[i]>limit,则i++
      - 如果 arr[i] < limit，则说明该位置需要加成   =》 贪心： 让加成的左边界压在i上（因为左边不需要了，所以最大面积覆盖的方式就是左边界压在i，剩余部分加成后面的数字）
        - 计算需要的 将军数量 （数学公式，注意向上取整 limit-arr[i] / 加成  ==》 向上取整）
      - 当发现 i 需要加成，但是 剩余将军数量为 0了， 那就返回 false，表明 这个limit做不到
      - 有将军就一直往后加成，如果来到最后了，都没有出现将军不够的情况，返回 true
  - 类似于 AOE 魔法师的贪心策略 -> 左边界压到点（每次从左边界压到点去尝试，然后 沿着下一组的开头继续压着左边界）
  - 线段树
...
```
[440. 字典序的第K小数字](topInterviewHardQuestions/FindKthNumberInDictionaryOrder.java)
- O(logN ^ 2) 的方法 -> 具体看代码65
- 模拟前缀树 -> 前缀树 前序遍历的顺序 就是 字典序！！
  - 但是 不需要 真正的创建 前缀树
  - 例如 1节点下面 分支为 10 11 12 13 ... 19
    - 10 下面的分支 100 101 102 103 104 ... 109
- Step1 : 首先构造一个函数，能够获得当前节点为头的整棵树 包含多少个子节点。
  - first = first * 10 记录最左， last = last * 10 + 9 记录最右
- 决定向右走/向下走  ==》 假设寻找第k个，那么要转化成寻找 k-1 下标，才适用下面规则 （不然的话，需要稍作修改）
  - （1）如果当前index >= 子树的size,那么向右走， index -= 子树的size
  - （2）如果当前 index < 子树的size，那么向下走, index--
- 最终，当 index == 0 时退出循环， 此时所在的节点就是 第k个， 也就是 k-1下标的那一个
